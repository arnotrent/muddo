...
def agent_dashboard(request):
    ...
    kpis = [
        ('truck','#1565c0','#e3f2fd', my_requests.count(), 'My Requests'),
        ('clock','#f57c00','#fff8e1', pending_count, 'Pending'),
        ('comment','#c62828','#fce8e2', unread, 'Unread Messages'),
        ('flask','#2e7d32','#e8f5e9', Product.objects.count(), 'Products in Catalogue'),
    ]
...
def agent_report_pdf(request,aid):
    ...
    DARK=colors.HexColor('#0d2b14'); MID=colors.HexColor('#2d6e35'); WHT=colors.white; LGRY=colors.HexColor('#f5f5f5'); LGRN=colors.HexColor('#e8f5e9'); GOLD=colors.HexColor('#c8a84b'); MUTED=colors.HexColor('#565656')
